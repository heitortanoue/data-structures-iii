...
int criaFiltro(Busca *filtros, int qtd){
	...
	filtros->tipo_campo = malloc(sizeof(int) * qtd); //id dos campos
	filtros->campo = malloc(sizeof(char *) * qtd);  //campos string
    for (int i = 0; i < qtd; i++){
        filtros->campo[i] = malloc(sizeof(char) * 128);
    }
	filtros->criterios = malloc(sizeof(char *) * qtd); //valores string
    for (int i = 0; i < qtd; i++){
